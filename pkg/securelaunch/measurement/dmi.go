 ...
 * calls smbios package from go-smbios to get all smbios data.
 * filters smbios data returned from go-smbios based on type provided in policy file.
 * The filtered data is then measured into the tpmHandle (tpm device).
 */
func (s *DmiCollector) Collect(tpmHandle io.ReadWriteCloser) error {
	...
	for _, k := range data { // k ==> data for each dmi type
		...
		for _, label := range labels {
			...
			slaunch.Debug("DMI Collector: Hashing %s information\n", label)
			b := new(bytes.Buffer)
			...
			if e := tpm.ExtendPCRDebug(tpmHandle, pcr, b.Bytes()); e != nil {
